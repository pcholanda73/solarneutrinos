...
c      write(99,*)cosphi,pphi(icos)
c      write(*,*)cosphi,p2e(tg,dm,cosphi)
      enddo
c      stop
...
c----       dividindo a noite em 6 regioes
c---- e cada regiao em <nkb> pontos (no cos(theta))
	delcos = (0.975-0.840)/float(ncore)
	if(delcos.ge.1.e-2)then              !100 pontos em lt#.dat
	  nkb = 100
	else
	  nkb = 1./delcos 
	endif
...
c..............................
       ...
       do 12 jr = 1, n_passos
          ...
          rho=rho_(ii)
          dmrho=sqrt((dm*c_2t-rho)**2.+(dm*s_2t)**2.)
          dmrho=sign(dmrho,dm)
          dm1=rho-dmrho
          dm2=rho+dmrho
